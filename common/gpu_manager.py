...
def check_gpus():
    ...
    # =============================================================================
    first_gpus = (os.popen("nvidia-smi --query-gpu=index --format=csv,noheader"
                           ).readlines()[0].strip())
    if not first_gpus == "0":
        print(
...
if check_gpus():
    ...
    def parse(line, qargs):
        ...
        def process(k, v):
            return ((int(to_numberic(v)) if power_manage_enable(v) else 1)
                    if k in numberic_args else v.strip())

        return {
            k: process(k, v)
            for k, v in zip(qargs,
                            line.strip().split(","))
        }
    ...
    def query_gpu(qargs=None):
        ...
        qargs_cmd = ",".join(qargs)
        cmd = f"nvidia-smi --query-gpu={qargs_cmd} --format=csv,noheader"
    ...
    def by_power(d):
        ...
        if any(v == 1 for v in power_infos):
            gpu_index = d["index"]
            print(f"Power management unable for GPU {gpu_index}")
            return 1
    ...
    class GPUManager:
        ...
        def auto_choice_gpuid(self, mode=0):
            """
            mode:
            ...
            if mode == 0:
                ...
                print(
                    "Choosing the GPU device has highest free memory rate...")
        ...
        def auto_choice_device(self, mode=0):
            gpu_id = self.auto_choice_gpuid()
            return tf.device("/gpu:{}".format(gpu_id))
